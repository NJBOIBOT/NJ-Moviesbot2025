...
class Database:
    ...
    async def add_verify_request(self, user_id, file_id):
        await self.verify.update_one({"_id": user_id}, {"$set": {"file_id": file_id}}, upsert=True)

    async def get_verify_request(self, user_id):
        user = await self.verify.find_one({'_id': user_id})
        return user.get('file_id', None) if user else None

    async def delete_verify_request(self, user_id):
        await self.verify.delete_one({'_id': user_id})
