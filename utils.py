...
async def get_poster(query, bulk=False, id=False, file=None):
    if not id:
        ...
        try:
            ...
            logger.warning("IMDb search failed.")
            return None
        if not movieid:
            return None
        if year:
    ...
    try:
        ...
        logger.warning("IMDb movie details lookup failed.")
        return None
    if not movie:
        return None
...
async def get_audio_info(client, file_id):
    temp_file_path = f"temp_{file_id}"
    try:
        # Download a small chunk of the file
        with open(temp_file_path, "wb") as file:
            async for chunk in client.stream_media(file_id, limit=1):
                file.write(chunk)
